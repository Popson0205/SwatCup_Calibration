...
# ── Constants ─────────────────────────────────────────────────────────────────
MAX_UPLOAD_MB   = int(os.getenv("SUFI2_MAX_UPLOAD_MB", "50"))
...
async def startup():
    ...
    log.info(f"Hydro-Modeller API v2.1.0 started | DEV_MODE={DEV_MODE} | MAX_UPLOAD={MAX_UPLOAD_MB}MB | JOB_TTL={JOB_TTL_HOURS}h")
...
async def health(request: Request):
    ...
    with JOBS_LOCK:
        ...
        total  = len(JOBS)
    return {"status": "ok", "version": "2.1.0", "jobs": {"active": active, "total": total}}
...
async def restart_job(job_id: str, background_tasks: BackgroundTasks):
    ...
    with job._lock:
        ...
        job.logs       = [f"[{time.strftime('%H:%M:%S')}] 🔄 Job restarted."]
        job.milestones = []
        job.result     = None
        job.error      = None
...
async def download_results(job_id: str):
    ...
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        # Include input files under input_files/ subfolder
...
async def get_result_image(job_id: str, path: str):
    """Serve a chart file (PNG or JSON) from the results directory."""
    from fastapi.responses import FileResponse as FR
    job = _get_job(job_id)
    results_dir = job.work_dir / "sufi2_results"
    img_path = (results_dir / path).resolve()
    try:
        img_path.relative_to(results_dir.resolve())
    except ValueError:
