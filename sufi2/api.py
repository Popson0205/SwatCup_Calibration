...
async def get_parameters():
    """Full SWAT parameter catalogue — 71 parameters across 12 categories."""
...
async def get_result_image(job_id: str, path: str):
    """Serve a PNG/image from the results directory. path is relative to sufi2_results/."""
    from fastapi.responses import FileResponse as FR
    job = _get_job(job_id)
    results_dir = job.work_dir / "sufi2_results"
    img_path = (results_dir / path).resolve()
    # Security: must be inside results_dir
    try:
