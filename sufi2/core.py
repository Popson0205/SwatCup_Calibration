...
def _save_png(fig, path: str):
    """Save a plotly figure as PNG using matplotlib as renderer (no kaleido needed)."""
    try:
        import io as _io
        # Use plotly's built-in static image export
        # Falls back gracefully if orca/kaleido not available
        try:
            ...
            # Simple fallback - just save a placeholder noting chart is in JSON
            fig_mpl, ax = plt.subplots(figsize=(10, 4))
            ax.text(0.5, 0.5, f"Chart saved as JSON\nOpen the .json file for interactive view",
