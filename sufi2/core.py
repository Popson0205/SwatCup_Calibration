...
# ── Chart helpers ────────────────────────────────────────────────────────────
def _save_png(fig, path: str):
    """
    Render a Plotly figure as a real PNG using matplotlib.
    No kaleido, no orca, no external binaries required.
    Extracts traces from the Plotly figure dict and redraws with matplotlib Agg.
    """
    try:
        import matplotlib
        matplotlib.use("Agg")
        ...
        title    = (layout.get("title") or {}).get("text", "") if isinstance(layout.get("title"), dict) else layout.get("title", "")

        # Count subplots
        n_rows = layout.get("grid", {}).get("rows", 1) if layout.get("grid") else 1
        n_cols = layout.get("grid", {}).get("columns", 1) if layout.get("grid") else 1
        if n_rows * n_cols < len(traces):
            n_rows, n_cols = 1, 1  # fallback to single panel

        fig_h = max(4, n_rows * 3.5)
        fig_w = max(10, n_cols * 5)
        fig_mpl, axes = plt.subplots(n_rows, n_cols, figsize=(fig_w, fig_h),
        ...
        COLORS = ["#2f81f7","#ff7b72","#3fb950","#d29922","#a5d6ff",
                  "#ffa657","#bc8cff","#58a6ff","#7ee787","#f0883e"]

        def _ax_for(trace, idx):
            """Map trace to subplot axis."""
            xref = trace.get("xaxis","x")
            yref = trace.get("yaxis","y")
            try:
                xi = int(xref.replace("x","") or "1") - 1
                yi = int(yref.replace("y","") or "1") - 1
                ai = yi * n_cols + xi
                return axes_flat[min(ai, len(axes_flat)-1)]
            except Exception:
                return axes_flat[min(idx, len(axes_flat)-1)]
        ...
        for i, trace in enumerate(traces):
            ...
            try:
                if ttype in ("scatter", "scattergl"):
                    ...
                    mc = marker.get("color", col)
                    # Use first colour if array
                    if isinstance(mc, list): mc = col
                    if "lines" in mode:
                        lw = (trace.get("line") or {}).get("width", 1.5)
                        ax.plot(x, y, color=mc, linewidth=lw, label=name, alpha=0.9)
                    elif "markers" in mode:
                        ax.scatter(x, y, c=mc, s=max(2, marker.get("size", 4)),
                                   alpha=0.6, label=name, edgecolors="none")
                    else:
                        ax.plot(x, y, color=mc, linewidth=1.5, label=name)

                elif ttype == "bar":
                    ax.barh(x, y, color=col, alpha=0.85, label=name)
                    ax.set_xlabel(layout.get("xaxis", {}).get("title", {}).get("text", "") or "", color=TEXT, fontsize=8)

                elif ttype in ("filled", "area") or trace.get("fill"):
                    # Filled area (95PPU band)
                    fc = trace.get("fillcolor", "rgba(70,130,180,0.3)")
                    ax.fill_between(range(len(y)), y, alpha=0.3, color=col, label=name)

            except Exception as te:
                log.debug(f"Trace {i} render error: {te}")
                continue

            # Axis labels
            if layout.get("xaxis"):
                xt = layout["xaxis"].get("title")
                if isinstance(xt, dict): xt = xt.get("text","")
                if xt: ax.set_xlabel(xt, color=TEXT, fontsize=8)
            if layout.get("yaxis"):
                yt = layout["yaxis"].get("title")
                if isinstance(yt, dict): yt = yt.get("text","")
                if yt: ax.set_ylabel(yt, color=TEXT, fontsize=8)

            # Legend
            handles, labels = ax.get_legend_handles_labels()
            if labels:
                ax.legend(fontsize=7, facecolor=SURF, edgecolor=GRID,
                          labelcolor=TEXT, framealpha=0.8)

        # Hide empty subplots
        for k in range(len(traces), len(axes_flat)):
            axes_flat[k].set_visible(False)

        if title:
            fig_mpl.suptitle(title, color=TEXT, fontsize=10, y=1.01)
        ...
        log.warning(f"PNG export failed for {path}: {e}")

# ── Pure-numpy replacements for scipy.stats (avoids _arpack DLL) ─────────────
def _linregress(x, y):
    """Minimal linregress: returns (slope, intercept, r, p_value, stderr)."""
    ...
    r = ssxy / np.sqrt(ssxx * ssyy) if (ssxx * ssyy) > 0 else 0.0
    r = float(np.clip(r, -1.0, 1.0))
    df = n - 2
    if df > 0 and abs(r) < 1.0:
        t = r * np.sqrt(df / (1 - r**2))
        from math import lgamma, pi as _pi
        # two-tailed p-value via incomplete beta approximation
        x_b = df / (df + t**2)
        ...
        p = 0.0
    se = np.sqrt((ssyy - slope * ssxy) / (ssxx * df)) if (ssxx * df) > 0 else 0.0
    return slope, intercept, r, float(p), float(se)

def _rankdata(a):
    """Pure-numpy rank data (average method)."""
...
DEFAULT_PARAMS = pd.DataFrame([
    {"name": "SFTMP.bsn", "min": -5.0, "max": 5.0, "change_type": "v", "category": "Snow", "description": "Snowfall temperature (°C)"},
    {"name": "SMTMP.bsn", "min": -5.0, "max": 5.0, "change_type": "v", "category": "Snow", "description": "Snow melt base temperature (°C)"},
    {"name": "SMFMX.bsn", "min": 0.0, "max": 20.0, "change_type": "v", "category": "Snow", "description": "Max melt rate for snow during year (mm/°C-day)"},
    {"name": "SMFMN.bsn", "min": 0.0, "max": 20.0, "change_type": "v", "category": "Snow", "description": "Min melt rate for snow during year (mm/°C-day)"},
    {"name": "TIMP.bsn", "min": 0.01, "max": 1.0, "change_type": "v", "category": "Snow", "description": "Snow pack temperature lag factor"},
    {"name": "SNOCOVMX.bsn", "min": 0.0, "max": 500.0, "change_type": "v", "category": "Snow", "description": "Min snow water content for 100% snow cover (mm)"},
    {"name": "SNO50COV.bsn", "min": 0.0, "max": 1.0, "change_type": "v", "category": "Snow", "description": "Snow water equiv for 50% snow cover"},
    {"name": "SURLAG.bsn", "min": 0.05, "max": 24.0, "change_type": "v", "category": "Runoff", "description": "Surface runoff lag time (days)"},
    {"name": "ADJ_PKR.bsn", "min": 0.5, "max": 2.0, "change_type": "v", "category": "Runoff", "description": "Peak rate adjustment factor for sediment routing"},
    {"name": "SPCON.bsn", "min": 0.0001, "max": 0.01, "change_type": "v", "category": "Sediment", "description": "Linear parameter for channel sediment routing"},
    {"name": "SPEXP.bsn", "min": 1.0, "max": 2.0, "change_type": "v", "category": "Sediment", "description": "Exponent parameter for channel sediment routing"},
    {"name": "ESCO.bsn", "min": 0.0, "max": 1.0, "change_type": "v", "category": "Evaporation", "description": "Soil evaporation compensation factor (basin level)"},
    {"name": "EPCO.bsn", "min": 0.0, "max": 1.0, "change_type": "v", "category": "Evaporation", "description": "Plant uptake compensation factor (basin level)"},
    {"name": "EVLAI.bsn", "min": 0.0, "max": 10.0, "change_type": "v", "category": "Evaporation", "description": "Leaf area index for ET"},
    {"name": "FFCB.bsn", "min": 0.0, "max": 1.0, "change_type": "v", "category": "Soil", "description": "Initial soil water storage expressed as fraction of field capacity"},
    {"name": "ESCO.hru", "min": 0.0, "max": 1.0, "change_type": "v", "category": "Evaporation", "description": "Soil evaporation compensation factor"},
    {"name": "EPCO.hru", "min": 0.0, "max": 1.0, "change_type": "v", "category": "Evaporation", "description": "Plant uptake compensation factor"},
    {"name": "OV_N.hru", "min": 0.01, "max": 30.0, "change_type": "v", "category": "Runoff", "description": "Manning's n for overland flow"},
    {"name": "LAT_TTIME.hru", "min": 0.0, "max": 180.0, "change_type": "v", "category": "Lateral Flow", "description": "Lateral flow travel time (days)"},
    {"name": "SLSUBBSN.hru", "min": 10.0, "max": 150.0, "change_type": "v", "category": "Runoff", "description": "Average slope length (m)"},
    {"name": "HRU_SLP.hru", "min": 0.0, "max": 0.6, "change_type": "r", "category": "Runoff", "description": "Average slope steepness (m/m)"},
    {"name": "CANMX.hru", "min": 0.0, "max": 100.0, "change_type": "v", "category": "Evaporation", "description": "Maximum canopy storage (mm)"},
    {"name": "RSDIN.hru", "min": 0.0, "max": 10000.0, "change_type": "v", "category": "Sediment", "description": "Initial residue cover (kg/ha)"},
    {"name": "CN2.mgt", "min": -0.25, "max": 0.25, "change_type": "r", "category": "Runoff", "description": "SCS runoff curve number (relative change)"},
    {"name": "USLE_P.mgt", "min": 0.1, "max": 1.0, "change_type": "v", "category": "Sediment", "description": "USLE support practice factor"},
    {"name": "BIOMIX.mgt", "min": 0.0, "max": 1.0, "change_type": "v", "category": "Soil", "description": "Biological mixing efficiency"},
    {"name": "ALPHA_BF.gw", "min": 0.0, "max": 1.0, "change_type": "v", "category": "Groundwater", "description": "Baseflow alpha factor (days)"},
    {"name": "GW_DELAY.gw", "min": 0.0, "max": 500.0, "change_type": "v", "category": "Groundwater", "description": "Groundwater delay (days)"},
    {"name": "GWQMN.gw", "min": 0.0, "max": 5000.0, "change_type": "v", "category": "Groundwater", "description": "Threshold depth for return flow (mm)"},
    {"name": "GW_REVAP.gw", "min": 0.02, "max": 0.2, "change_type": "v", "category": "Groundwater", "description": "Groundwater revap coefficient"},
    {"name": "REVAPMN.gw", "min": 0.0, "max": 500.0, "change_type": "v", "category": "Groundwater", "description": "Threshold depth for revap (mm)"},
    {"name": "RCHRG_DP.gw", "min": 0.0, "max": 1.0, "change_type": "v", "category": "Groundwater", "description": "Deep aquifer percolation fraction"},
    {"name": "GWHT.gw", "min": 0.0, "max": 25.0, "change_type": "v", "category": "Groundwater", "description": "Initial groundwater height (m)"},
    {"name": "GW_SPYLD.gw", "min": 0.0, "max": 0.4, "change_type": "v", "category": "Groundwater", "description": "Specific yield of the shallow aquifer"},
    {"name": "SHALLST.gw", "min": 0.0, "max": 50000.0, "change_type": "v", "category": "Groundwater", "description": "Initial depth of water in shallow aquifer (mm)"},
    {"name": "DEEPST.gw", "min": 0.0, "max": 50000.0, "change_type": "v", "category": "Groundwater", "description": "Initial depth of water in deep aquifer (mm)"},
    {"name": "ALPHA_BF_D.gw", "min": 0.0, "max": 1.0, "change_type": "v", "category": "Groundwater", "description": "Baseflow alpha factor for bank storage (days)"},
    {"name": "SOL_Z.sol", "min": -0.5, "max": 0.5, "change_type": "r", "category": "Soil", "description": "Depth from soil surface to bottom of layer (mm)"},
    {"name": "SOL_AWC.sol", "min": -0.5, "max": 0.5, "change_type": "r", "category": "Soil", "description": "Available water capacity of soil layer (mm/mm)"},
    {"name": "SOL_K.sol", "min": -0.8, "max": 0.8, "change_type": "r", "category": "Soil", "description": "Saturated hydraulic conductivity (mm/hr)"},
    {"name": "SOL_BD.sol", "min": -0.5, "max": 0.6, "change_type": "r", "category": "Soil", "description": "Moist bulk density (g/cm³)"},
    {"name": "SOL_ALB.sol", "min": 0.0, "max": 0.25, "change_type": "v", "category": "Soil", "description": "Moist soil albedo"},
    {"name": "USLE_K.sol", "min": 0.1, "max": 0.65, "change_type": "r", "category": "Sediment", "description": "USLE soil erodibility factor"},
    {"name": "SOL_CBN.sol", "min": -0.5, "max": 0.5, "change_type": "r", "category": "Soil", "description": "Organic carbon content (%)"},
    {"name": "CH_N2.rte", "min": 0.01, "max": 0.3, "change_type": "v", "category": "Channel", "description": "Manning's n for main channel"},
    {"name": "CH_K2.rte", "min": 0.0, "max": 500.0, "change_type": "v", "category": "Channel", "description": "Effective hydraulic conductivity of main channel (mm/hr)"},
    {"name": "CH_W2.rte", "min": -0.5, "max": 0.5, "change_type": "r", "category": "Channel", "description": "Average width of main channel (m)"},
    {"name": "CH_D2.rte", "min": -0.5, "max": 0.5, "change_type": "r", "category": "Channel", "description": "Average depth of main channel (m)"},
    {"name": "CH_S2.rte", "min": -0.5, "max": 0.5, "change_type": "r", "category": "Channel", "description": "Average slope of main channel (m/m)"},
    {"name": "CH_EROD.rte", "min": 0.0, "max": 1.0, "change_type": "v", "category": "Sediment", "description": "Channel erodibility factor"},
    {"name": "CH_COV.rte", "min": 0.0, "max": 1.0, "change_type": "v", "category": "Sediment", "description": "Channel cover factor"},
    {"name": "CH_N1.rte", "min": 0.01, "max": 30.0, "change_type": "v", "category": "Channel", "description": "Manning's n for tributary channels"},
    {"name": "CH_K1.rte", "min": 0.0, "max": 300.0, "change_type": "v", "category": "Channel", "description": "Effective hydraulic conductivity of tributary channel (mm/hr)"},
    {"name": "TLAPS.sub", "min": -10.0, "max": 10.0, "change_type": "v", "category": "Climate", "description": "Temperature lapse rate (°C/km)"},
    {"name": "PLAPS.sub", "min": -500.0, "max": 500.0, "change_type": "v", "category": "Climate", "description": "Precipitation lapse rate (mm/km)"},
    {"name": "CH_N1.sub", "min": 0.01, "max": 30.0, "change_type": "v", "category": "Channel", "description": "Manning's n for tributary channels (subbasin)"},
    {"name": "CH_K1.sub", "min": 0.0, "max": 300.0, "change_type": "v", "category": "Channel", "description": "Hydraulic conductivity of tributary channel (subbasin)"},
    {"name": "PND_K.pnd", "min": 0.0, "max": 2.0, "change_type": "v", "category": "Pond", "description": "Hydraulic conductivity of pond bottom (mm/hr)"},
    {"name": "PND_N.pnd", "min": 0.01, "max": 0.3, "change_type": "v", "category": "Pond", "description": "Manning's n for pond"},
    {"name": "MUMAX.wwq", "min": 1.0, "max": 3.0, "change_type": "v", "category": "Water Quality", "description": "Max specific algal growth rate (day⁻¹)"},
    {"name": "AI0.wwq", "min": 50.0, "max": 1000.0, "change_type": "v", "category": "Water Quality", "description": "Ratio of chlorophyll a to algal biomass"},
    {"name": "K_N.wwq", "min": 0.01, "max": 2.0, "change_type": "v", "category": "Water Quality", "description": "Michaelis-Menton half-saturation constant for nitrogen"},
    {"name": "K_P.wwq", "min": 0.001, "max": 0.1, "change_type": "v", "category": "Water Quality", "description": "Michaelis-Menton half-saturation constant for phosphorus"},
    {"name": "ALPHA_BNK.rte", "min": 0.0, "max": 1.0, "change_type": "v", "category": "Groundwater", "description": "Baseflow alpha factor for bank storage"},
    {"name": "SOL_NO3.sol", "min": 0.0, "max": 100.0, "change_type": "v", "category": "Nutrients", "description": "Initial NO3 concentration in soil layer (mg/kg)"},
    {"name": "SOL_ORGN.sol", "min": 0.0, "max": 10000.0, "change_type": "v", "category": "Nutrients", "description": "Initial organic N in soil layer (mg/kg)"},
    {"name": "SOL_LABP.sol", "min": 0.0, "max": 200.0, "change_type": "v", "category": "Nutrients", "description": "Initial labile P in soil layer (mg/kg)"},
    {"name": "SOL_ORGP.sol", "min": 0.0, "max": 4000.0, "change_type": "v", "category": "Nutrients", "description": "Initial organic P in soil layer (mg/kg)"},
    {"name": "LAT_SED.hru", "min": 0.0, "max": 5000.0, "change_type": "v", "category": "Sediment", "description": "Sediment concentration in lateral flow (mg/L)"},
    {"name": "FILTERW.hru", "min": 0.0, "max": 100.0, "change_type": "v", "category": "Sediment", "description": "Width of edge-of-field filter strip (m)"},
    {"name": "DEP_IMP.hru", "min": 0.0, "max": 6000.0, "change_type": "v", "category": "Soil", "description": "Depth to impervious layer (mm)"},
])
...
def sensitivity_analysis(samples: np.ndarray, obj_vals: np.ndarray, names: List[str]) -> pd.DataFrame:
    ranked_obj = _rankdata(obj_vals)
    rows = []
    for i, name in enumerate(names):
        ranked_p = _rankdata(samples[:, i])
        slope, _, _, p_val, se = _linregress(ranked_p, ranked_obj)
...
def plot_dotty(samples, obj_vals, names, label, out):
    ...
    fig = ps.make_subplots(rows=nrows, cols=ncols, subplot_titles=names)
    vmin, vmax = float(np.percentile(obj_vals, 5)), float(np.percentile(obj_vals, 95))
    for j, name in enumerate(names):
        r, c = divmod(j, ncols)
        fig.add_trace(go.Scatter(
            x=samples[:, j].tolist(), y=obj_vals.tolist(),
            mode="markers",
            marker=dict(color=obj_vals.tolist(), colorscale="RdYlGn",
                        cmin=vmin, cmax=vmax, size=4, opacity=0.6),
            name=name, showlegend=False,
        ), row=r+1, col=c+1)
    fig.update_layout(title_text=f"Dotty Plots — {label}", height=400*nrows)
    _base = str(out).replace(".png", "")
    with open(_base + ".json", "w") as fh:
        fh.write(fig.to_json())
    _save_png(fig, _base + ".png")
    gc.collect()

def plot_ppu(ppu, reach_id, label, out):
    dates = [str(d) for d in ppu["dates"]]
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=dates+dates[::-1],
        y=list(ppu["upper"])+list(ppu["lower"])[::-1],
        fill="toself", fillcolor="rgba(70,130,180,0.3)",
        line=dict(color="rgba(255,255,255,0)"), name="95PPU"))
    fig.add_trace(go.Scatter(x=dates, y=list(ppu["best"]),
        line=dict(color="darkorange", width=1.5), name="Best sim"))
    fig.add_trace(go.Scatter(x=dates, y=list(ppu["obs"]),
        mode="markers", marker=dict(color="black", size=3), name="Observed"))
    fig.update_layout(
        title=f"95PPU — Reach {reach_id} | {label} | P-factor:{ppu['p_factor']:.3f}  R-factor:{ppu['r_factor']:.3f}",
        xaxis_title="Date", yaxis_title="Flow (m³/s)", height=400)
    _base = str(out).replace(".png", "")
    with open(_base + ".json", "w") as fh:
        fh.write(fig.to_json())
    _save_png(fig, _base + ".png")
...
def plot_sensitivity(sens_df, label, out):
    ...
    fig = go.Figure(go.Bar(
        x=df["t-stat"].abs().tolist(), y=df["Parameter"].tolist(),
        orientation="h", marker_color=colors,
        hovertemplate="%{y}: %{x:.3f}<extra></extra>"))
    fig.add_vline(x=1.96, line_dash="dash", line_color="gray")
    fig.update_layout(
        title=f"Sensitivity — {label}",
...
def plot_convergence(conv_data: dict, reach_ids: List[int], obj_label: str, out: Path):
    ...
    titles = ["Combined"] + [f"Reach {r}" for r in reach_ids]
    fig = ps.make_subplots(rows=nrows, cols=ncols, subplot_titles=titles)
    fig.add_trace(go.Scatter(
        x=iters, y=[conv_data[i]["combined_best"] for i in iters],
        mode="lines+markers", line=dict(color="steelblue", width=2), name="Combined"),
        row=1, col=1)
    fig.add_hline(y=0.5, line_dash="dash", line_color="green", row=1, col=1)
    fig.add_hline(y=0.65, line_dash="dash", line_color="darkgreen", row=1, col=1)
    for idx, rid in enumerate(reach_ids):
        r, c = divmod(idx + 1, ncols)
        fig.add_trace(go.Scatter(
            x=iters, y=[conv_data[i]["reach_best"].get(rid, None) for i in iters],
            mode="lines+markers", line=dict(color="darkorange", width=2), name=f"Reach {rid}"),
            row=r+1, col=c+1)
        fig.add_hline(y=0.5, line_dash="dash", line_color="green", row=r+1, col=c+1)
    fig.update_layout(title_text=f"Best {obj_label} per Iteration", height=400*nrows, showlegend=False)
    _base = str(out).replace(".png", "")
    with open(_base + ".json", "w") as fh:
        fh.write(fig.to_json())
    _save_png(fig, _base + ".png")
...
def plot_validation(obs_dict, val_sims, reach_ids, out_dir):
    for rid in reach_ids:
        ...
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=dates, y=obs.loc[common].values.tolist(),
            line=dict(color="black", width=1.5), name="Observed"))
        fig.add_trace(go.Scatter(x=dates, y=sim.loc[common].values.tolist(),
            line=dict(color="steelblue", width=1.5), name="Simulated (best params)"))
        fig.update_layout(
            title=f"Validation — Reach {rid} | NSE: {nse_v:.3f}",
            xaxis_title="Date", yaxis_title="Flow (m³/s)", height=400)
        _base = str(out_dir / f"validation_rch{rid}")
        with open(_base + ".json", "w") as fh:
            fh.write(fig.to_json())
        _save_png(fig, _base + ".png")
        gc.collect()
...
class SUFI2Engine:
    ...
    def calibrate(self) -> dict:
        ...
        obs_dict = {r: s.loc[calib_s:calib_e] for r, s in self._obs_dict_full.items() if r in reach_ids}
    ...
    def validate(self) -> dict:
        ...
        obs_valid = {r: s.loc[valid_s:valid_e] for r, s in self._obs_dict_full.items() if r in reach_ids
