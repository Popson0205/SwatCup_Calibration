...
class SUFI2Engine:
    ...
    def _setup(self):
        ...
        # ── Validate date overlap between sim and calibration period ──────
        calib_start = pd.Timestamp(self.cfg.periods.calibration.start)
        calib_end   = pd.Timestamp(self.cfg.periods.calibration.end)
        for rid, sim_s in base_sims.items():
            obs_s   = self._obs_dict_full.get(rid)
            overlap = sim_s.loc[calib_start:calib_end]
            if len(overlap) == 0:
                log.warning(
                    "Reach %d: output.rch has NO data in calibration period %s → %s. "
                    "Sim range: %s → %s. NSE will be -inf.",
                    rid, calib_start.date(), calib_end.date(),
                    sim_s.index[0].date() if len(sim_s) > 0 else "?",
                    sim_s.index[-1].date() if len(sim_s) > 0 else "?",
                )
                self._emit(
                    f"WARNING: Reach {rid} — output.rch has no data in calibration period "
                    f"({calib_start.date()} → {calib_end.date()}). "
                    f"Sim covers {sim_s.index[0].date() if len(sim_s)>0 else '?'} → "
                    f"{sim_s.index[-1].date() if len(sim_s)>0 else '?'}. "
                    "Check output.rch dates and file.cio start year.",
                    0.07,
                )
    ...
    def calibrate(self) -> dict:
        ...
        if global_best_set is None:
            raise RuntimeError(
                "Calibration produced no valid simulations — all objective values are -inf. "
                "This usually means the simulated output (output.rch) dates do not overlap "
                "with the calibration period. Check that:\n"
                f"  - output.rch covers {cfg.periods.calibration.start} → {cfg.periods.calibration.end}\n"
                f"  - observed_flow_rchN.csv covers the same period\n"
                f"  - file.cio start year matches your simulation start"
            )
